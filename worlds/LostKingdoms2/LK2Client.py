...
def _give_item(ctx: LK2Context, item_name: str) -> bool:
    ...
    """
    item_data = ctx.cursor.execute("Select DolphinAddress from cards where name = ?", (str,)).fetchone()
    if item_data is not None:
        memory_address = int(item_data[0], 16)
        ...
        logger.error("Received Invalid Item:" + item_name)
        return False
...
def main(connect: Optional[str] = None, password: Optional[str] = None) -> None:
    """
    Run the main async loop for the Lost Kingdoms 2 client.

    :param connect: Address of the Archipelago server.
    :param password: Password for server authentication.
    """
    Utils.init_logging("The Lost Kingdoms 2 Client")
